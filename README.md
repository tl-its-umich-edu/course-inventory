...
3. Ensure you have one function or method defined that will kick off all other steps in the job.
   ...
   used during the job, and a timestamp of when that data was updated (or collected).
    ...
    For convenience, a pandas `Timestamp` object may be generated by [the method `pd.to_datetime`]
    (https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.to_datetime.html).
    This accepts a number of time formats and objects and will return a `pd.Timestamp` object for
    single values.  See the `run_course_inventory` function in the `COURSE_INVENTORY` job for an
    example.
